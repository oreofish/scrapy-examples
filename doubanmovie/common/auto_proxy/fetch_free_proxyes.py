...
def fetch_kxdaili(page):
    ...
    try:
        ...
        logger.warning("fail to fetch from kxdaili: %s" % e)
    return proxyes
...
def fetch_mimvp():
    ...
    try:
        ...
        logger.warning("fail to fetch from mimvp: %s" % e)
    return proxyes
...
def fetch_all(endpage=2):
    ...
    for p in proxyes:
        if check(p):
            logger.info("valid proxy %s" % p)
            valid_proxyes.append(p)
    logger.info("fetched %d valid proxies" % len(valid_proxyes))
